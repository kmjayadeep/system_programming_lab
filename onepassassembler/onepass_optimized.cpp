...
class Line{
public:
	Line(string l,string oc,string op) : label(l), opCode(oc), operand(op){}
...
Line readLine(ifstream &f){
	...
	if(c=='\t')
		...
		f.seekg(-1,f.cur); f>>label;
	}
	f>>opcode>>operand;
	return Line(label,opcode,operand);
}
class Op{ public:
	int value;	list<int> addresses;
	Op() : value(-1){}
};
class TextRecord{ public:
	int length,start;	string record;
	TextRecord(int s) : start(s), length(0){}
	void push(string t){
		record+="^"+t;
		length+=t.length();
	}
...
int main(int argc,char *argv[]){
	...
	while(line.opCode!="END"){
		...
		if(line.label!=""){
			...
			if(addresses.size()!=0){
				...
				for(list<int>::iterator i = addresses.begin();i!=addresses.end();i++){
					stringstream s;
					s<<setw(4)<<setfill('0')<<hex<<locCtr; tRecord = new TextRecord(*i+1);
					tRecord->push(s.str());
					tRecord->write(outFile);
		...
		if(opTab.find(line.opCode)!=opTab.end()){
			...
			locCtr+=len;
			tRecord->write(outFile); tRecord = new TextRecord(locCtr);
			...
			if(line.operand[0]=='X'){
				...
				for(int i=0;i<op.length();i++) s<<hex<<(int) op[i];
			}else
				s<<hex<<line.operand;
			objCode = s.str();
